...
Skills cần load từ skill lookup của `nli-tools` đang kết nối:
- instructor
- prompt_refinement
- validator
...
6. Lưu một verdict file cho mỗi model tại:
...
2. Nếu decision=needs_prompt_update, dừng execution tự động. Inspect
   disagreement_rows.csv đã log, prompt snapshots, verdict files, và calibration
   rows.
3. Báo cáo rejected sample count, disagreement evidence, và next step nhỏ nhất có
...
- Validator subagents không gọi MCP tools hoặc ghi runtime state.
